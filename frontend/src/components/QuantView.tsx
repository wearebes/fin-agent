...
const copy = {
  zh: {
    eyebrow: 'QUANT FORENSICS / 策略法医', titleA: '先证明策略', titleB: '没有骗你',
    desc: '不追求漂亮曲线。用真实历史行情检查未来数据泄漏、过拟合、参数脆弱性、交易成本、市场状态依赖与伪 Alpha。',
    ...
    running: '正在获取行情并审计…', presets: '策略模板', introTitle: '一次审计，六份证据',
  ...
  en: {
    eyebrow: 'QUANT FORENSICS / STRATEGY AUTOPSY', titleA: 'Prove the strategy',
    titleB: 'is not fooling you',
    desc: 'No vanity curves. Audit look-ahead leakage, overfitting, parameter fragility, costs, regime dependence, and false alpha on real historical prices.',
    ...
    running: 'Fetching prices and auditing…', presets: 'Templates', introTitle: 'One audit. Six lines of evidence.',
...
export default function QuantView() {
  ...
  const mutation = useMutation({ mutationFn: runForensics })
  ...
  return (
    <main className="qf-page">
      <div className="qf-ambient qf-ambient-a" /><div className="qf-ambient qf-ambient-b" />
      <section className="qf-hero">
        <div>
          <div className="qf-eyebrow"><ScanSearch size={15} />{labels.eyebrow}</div>
          <h1>{labels.titleA}<br /><span>{labels.titleB}</span></h1>
          <p>{labels.desc}</p>
        </div>
        <div className="qf-guardrail"><ShieldCheck size={21} /><div><strong>Evidence locked</strong><span>{labels.guardrail}</span></div></div>
      </section>

      <section className="qf-shell">
        ...
        {!mutation.data && !mutation.error && <Intro labels={labels} />}
        ...
        {mutation.data && <ScoreCard report={mutation.data} labels={labels} />}
      </section>

      {mutation.data && <Report report={mutation.data} lang={lang} labels={labels} />}
    </main>
  )
}

function Intro({ labels }: { labels: Labels }) {
  return <div className="qf-panel qf-intro"><div className="qf-scan"><div className="qf-scan-core"><ScanSearch size={38} /></div><span className="qf-orbit qf-orbit-a" /><span className="qf-orbit qf-orbit-b" /></div><span>LOCKED AUDIT PROTOCOL</span><h2>{labels.introTitle}</h2><p>{labels.introDesc}</p><div className="qf-intro-list">{['Leakage', 'Overfit', 'Sensitivity', 'Cost cliff', 'Regime', 'Alpha'].map((item, index) => <div key={item}><b>0{index + 1}</b>{item}</div>)}</div></div>
}
...
function Report({ report, lang, labels }: { report: ForensicsReport; lang: 'zh' | 'en'; labels: Labels }) {
  const metrics = [
  ...
  return <section className="qf-report">
    <div className="qf-report-head"><div><span>REPORT / {report.run_id}</span><h2>{report.ticker} · {report.strategy_label}</h2></div><div>{report.data_start} → {report.data_end}<br />{labels.source}</div></div>
    <div className="qf-metrics">{metrics.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div>
    <div className="qf-report-grid">
      <article className="qf-card qf-equity"><div className="qf-card-title"><div><span>02 / EVIDENCE</span><h3>{labels.curve}</h3></div><div className="qf-legend"><span><i className="strategy" />{labels.strategy}</span><span><i />{labels.base}</span></div></div><EquityChart report={report} /></article>
    ...
    <div className="qf-section-title"><div><span>03 / DIAGNOSIS</span><h2>{labels.checks}</h2></div><Gauge size={24} /></div>
    ...
    <div className="qf-analysis-grid">
      <article className="qf-card"><div className="qf-card-title"><div><span>04 / ROBUSTNESS</span><h3>{labels.sensitivity}</h3></div></div><div className="qf-bar-list">{report.sensitivity.map((point) => { const width = Math.min(100, Math.max(4, 50 + point.total_return_pct)); return <div key={point.label}><span>{point.label}<small>{point.fast_window}/{point.slow_window}</small></span><div><i style={{ width: `${width}%` }} /></div><strong>{formatPct(point.total_return_pct)}</strong></div>})}</div></article>
      <article className="qf-card"><div className="qf-card-title"><div><span>05 / EXECUTION</span><h3>{labels.costs}</h3></div></div><div className="qf-cost-list">{report.cost_scenarios.map((item) => <div key={item.cost_bps}><span>{item.cost_bps} bps</span><i /><strong>{formatPct(item.total_return_pct)}</strong></div>)}</div></article>
      <article className="qf-card"><div className="qf-card-title"><div><span>06 / REGIMES</span><h3>{labels.regimes}</h3></div></div><div className="qf-regimes">{report.regimes.map((item) => <div key={item.regime}><span>{regimeLabels[item.regime]?.[lang] || item.regime}</span><strong>{formatPct(item.annualized_return_pct)}</strong><small>{item.trading_days} {labels.observations}</small></div>)}</div></article>
    </div>
