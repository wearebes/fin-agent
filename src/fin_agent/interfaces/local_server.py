...
def local_settings(data_dir: Path, owner: str | None = None) -> AppSettings:
    ...
    settings.database.url = f"sqlite:///{(data_dir / 'fin-agent.db').as_posix()}"
...
def main() -> None:
    ...
    parser.add_argument("--owner", help="Existing local Fin-agent username allowed to use Codex")
    ...
    try:
        settings = local_settings(args.data_dir, args.owner)
