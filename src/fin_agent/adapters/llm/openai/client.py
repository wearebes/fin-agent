...
class OpenAIClient:
    ...
    def _make_client(self) -> AsyncOpenAI | None:
        api_key = (
            self._config.api_key.get_secret_value() if self._config.api_key is not None else None
        )
        self._has_api_key = bool(api_key)
        kwargs: dict[str, Any] = {
            # The SDK accepts an empty key. This keeps local setup available
            # before configuration; requests still fail safely if bypassed.
            "api_key": api_key or "",
    ...
    def reconfigure(self, config: OpenAIConfig) -> None:
        """Apply a local provider configuration without exposing its credential."""
        self._config = config
        self._client = self._make_client()
    ...
    async def check_connection(self) -> None:
        if not self._has_api_key or self._client is None:
