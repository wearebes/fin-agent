...
2026-09-12：修复 A 股代码路由、三张财报的字段映射及年度筛选，补充公司简介备用接口和免搜索密钥的新浪个股新闻（日期、原文链接、最多三篇正文节选）。图文研究自动补取年度利润表、资产负债表和现金流量表；累计中报数据不与单季度数据混合。公共来源仍可能限流或缺失；全网搜索需要另配 Tavily/Exa，未取到的内容不得编造。个人模型请求超时上调至 180 秒，失败记录保留错误类别；服务重启后须重新保存个人 API Key。
